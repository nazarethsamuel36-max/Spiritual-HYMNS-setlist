      ...
      <div className="mx-auto max-w-3xl">
        ...
        {authenticated && (
          <div className="grid gap-5 md:grid-cols-2">
            <section className="rounded-xl border border-slate-800 bg-slate-900 p-5 shadow-xl"><h2 className="text-xl font-bold">Add another phone</h2><p className="mt-2 text-sm leading-6 text-slate-400">Create a short-lived link and open it on the phone you want to trust.</p><button type="button" disabled={busy} onClick={() => run(async () => { const created = await createEnrollmentLink(); const url = `${window.location.origin}/?admin_enroll=${encodeURIComponent(created.token)}`; setLink(url); try { await copyText(url); } catch { setStatus('Link created. Press Copy link or press and hold the link to copy it.'); } }, 'Enrollment link copied. It expires in 10 minutes.')} className="mt-5 w-full rounded-lg bg-cyan-300 px-4 py-3 font-bold text-slate-950 disabled:opacity-40">Create enrollment link</button>{link && <><input readOnly value={link} onClick={(event) => event.currentTarget.select()} className="mt-3 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-300" /><div className="mt-2 grid grid-cols-2 gap-2"><button type="button" disabled={busy} onClick={() => run(async () => { await copyText(link); }, 'Enrollment link copied.')} className="rounded-lg border border-cyan-300 px-4 py-2 text-sm font-bold text-cyan-200 disabled:opacity-40">Copy link</button><button type="button" disabled={busy} onClick={() => run(async () => { await shareLink(link); }, 'Enrollment link shared.')} className="rounded-lg border border-slate-600 px-4 py-2 text-sm font-bold text-slate-200 disabled:opacity-40">Share link</button></div></>}</section>
