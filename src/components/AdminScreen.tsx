...
export function AdminScreen({ authenticated, onClose, onExit, onBootstrapped }: AdminScreenProps) {
  ...
  const [enrollmentToken] = useState(() => new URLSearchParams(window.location.search).get('admin_enroll') ?? '');
  ...
  const hasEnrollmentToken = enrollmentToken.trim().length > 0;
  ...
  return (
    <main className="min-h-screen bg-slate-950 px-4 py-8 text-slate-100 sm:px-8">
      <div className="mx-auto max-w-3xl">
        ...
        {!authenticated && !hasEnrollmentToken && (
        ...
        {(!authenticated && hasEnrollmentToken) && (
          <section className="rounded-xl border border-slate-800 bg-slate-900 p-5 shadow-xl"><h2 className="text-xl font-bold">Join as an admin device</h2><p className="mt-2 text-sm leading-6 text-slate-400">This invitation came from an existing admin phone. Choose a name for this phone and accept the invitation.</p><input value={deviceName} onChange={(event) => setDeviceName(event.target.value)} placeholder="Device name" className="mt-5 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 text-slate-100 outline-none focus:border-cyan-300" /><button type="button" disabled={busy || !deviceName.trim()} onClick={() => run(async () => { await acceptEnrollmentLink(enrollmentToken.trim(), deviceName); }, 'Enrollment accepted. Ask the existing admin phone to approve this device.')} className="mt-4 w-full rounded-lg bg-cyan-300 px-4 py-3 font-bold text-slate-950 disabled:opacity-40">Accept enrollment</button></section>
        )}
