    ...
    return (
      <div className={`flex-col min-h-full w-full bg-[#FAFAFA] flex ${langClass}`}>
        ...
        <div
          ...
          onPointerDown={readerMode === 'edit' ? undefined : handlePointerDown}
          onPointerMove={readerMode === 'edit' ? undefined : handlePointerMove}
          onPointerUp={readerMode === 'edit' ? undefined : handlePointerUp}
          onPointerCancel={readerMode === 'edit' ? undefined : handlePointerUp}
          style={{
            transform: readerMode === 'edit' ? undefined : (swipeOffset !== 0 ? `translateX(${swipeOffset * 0.3}px)` : undefined),
            transition: readerMode === 'edit' ? undefined : (swipeOffset === 0 ? 'transform 0.2s ease-out' : 'none'),
            touchAction: readerMode === 'edit' ? 'auto' : 'pan-y',
          }}
          className={`flex-1 overflow-y-auto w-full px-4 md:px-8 pt-8 pb-40 overscroll-contain ${readerMode === 'edit' ? '' : 'select-none'}`}
        >
