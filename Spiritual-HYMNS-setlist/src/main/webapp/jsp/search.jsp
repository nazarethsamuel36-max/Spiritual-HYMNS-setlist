...
<body class="bg-surface font-body text-on-surface flex flex-col min-h-screen">
    ...
    <main class="flex-grow max-w-[1920px] mx-auto w-full px-4 md:px-8 py-6 md:py-8">
        <h1 class="text-2xl font-headline font-extrabold tracking-tight text-on-surface mb-4 md:text-3xl">Search Results</h1>
        ...
        <c:if test="${not empty searchQuery}">
            <c:choose>
                <c:when test="${not empty searchResults}">
                    <p class="text-on-surface-variant font-medium mb-4"><c:out value="${searchResults.size()}"/> result${searchResults.size() != 1 ? 's' : ''} for "<strong class="text-primary"><c:out value="${searchQuery}"/></strong>"</p>

                    <!-- Song Grid -->
                    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-3 md:gap-4">
                        <c:forEach var="songResult" items="${searchResults}">
                            <a href="${pageContext.request.contextPath}/song?id=${songResult['id']}"
                               class="song-card group bg-surface-container-lowest p-4 rounded-xl transition-all duration-300 hover:-translate-y-1 hover:shadow-xl border border-surface-dim hover:border-primary/20 flex flex-col justify-between text-decoration-none"
                                ...
                                <div class="space-y-2">
                                    <div class="flex justify-between items-start">
                                        <div class="flex-grow">
                                            ...
                                            <h2 class="text-lg font-headline font-bold text-on-surface leading-tight group-hover:text-primary transition-colors"><c:out value="${songResult['title']}"/></h2>
                                        </div>
                                    ...
                                    <p class="text-xs font-semibold text-primary/80 tracking-wide uppercase"><c:out value="${songResult['artist']}"/></p>

                                    <div class="flex items-center gap-2 flex-wrap pt-1">
                                        <c:if test="${not empty songResult['originalKey']}">
                                            <div class="px-2 py-1 bg-primary-fixed text-on-primary-fixed text-[10px] font-bold uppercase tracking-widest rounded-md">
                                                Key of <c:out value="${songResult['originalKey']}"/>
                                            </div>
                                        </c:if>
                                        <c:if test="${not empty songResult['language']}">
                                            <div class="px-2 py-1 bg-tertiary-fixed text-on-tertiary-fixed text-[10px] font-bold uppercase tracking-widest rounded-md">
                                    ...
                                    <c:if test="${not empty songResult['matchedLine']}">
                                        <div class="mt-2 p-2 bg-surface-container text-xs font-mono text-on-surface-variant rounded-lg border border-outline-variant/30 leading-relaxed max-h-16 overflow-hidden relative">
                                            ...<c:out value="${songResult['matchedLine']}"/>...
                                            <div class="absolute bottom-0 left-0 right-0 h-6 bg-gradient-to-t from-surface-container to-transparent"></div>
                                        </div>
                                    </c:if>
                                </div>
                            </a>
                ...
                <c:otherwise>
                    <div class="col-span-full py-10 text-center bg-surface-container-low rounded-2xl border border-dashed border-outline-variant/40 max-w-3xl mx-auto">
